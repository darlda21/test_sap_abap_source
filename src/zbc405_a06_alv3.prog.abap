...
AT SELECTION-SCREEN ON VALUE-REQUEST FOR pa_lv.
  ...
  CALL FUNCTION 'LVC_VARIANT_SAVE_LOAD'   " variant 불러오기
   EXPORTING
...
*  gv_variant-variant = pa_lv. " f4를 사용하지 않고 직접 입력했을 때 적용되게 만드는 것. 이건 언제...?
  gv_save = 'U'.
ENDFORM.
*&---------------------------------------------------------------------*
*& Form make_layout
...
FORM make_layout .
  ...
  gs_layout-excp_fname = 'LIGHT'.       " light field name 설정. 설정 안해도 맨 앞으로 가는 필드
